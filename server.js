...
function fetch(url) {
  // Use allorigins.win as a CORS/IP proxy to bypass Bybit blocking
    ...
    const req = https.get(proxyUrl, {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          reject(new Error(`Parse failed: ${raw.substring(0, 100)}`));
        }
      });
    });
    req.on('error', reject);
...
async function fetchAPI(path) {
  ...
  for (const base of bases) {
    try {
      ...
      if (data && data.retCode === 0) return data;
      console.log(`  ${base} retCode: ${data?.retCode}`);
    } catch(e) {
      console.log(`  API ${base} failed: ${e.message.substring(0, 80)}`);
    }
